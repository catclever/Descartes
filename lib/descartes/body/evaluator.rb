  ...
  class Body
    module Evaluator
      ...
      def evaluate_condition_via_llm?(requirement, for_job, soul)
        @logger.debug "[Descartes] AI evaluating dependency: '#{requirement}' for Job :#{for_job.name}"
        ...
        case result["status"]
        when "true"
          @logger.info "[Descartes] AI Evaluated '#{requirement}': true"
          true
        when "missing_task"
          @logger.info "[Descartes] AI Evaluated '#{requirement}': missing_task. Spawning auto-planner for: #{result["suggested_goal"]}"
          ...
          false # Not met yet, let the planner work on it
        else
          @logger.info "[Descartes] AI Evaluated '#{requirement}': false"
          false
        end
      rescue StandardError => e
        @logger.error "[Descartes] Failed to evaluate AI condition '#{requirement}': #{e.message}. Raw output: #{result_text}"
        false
      end
    end
  end
