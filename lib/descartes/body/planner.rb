  ...
  class Body
    module Planner
      ...
      def spawn_dynamic_job_for_missing_requirement(soul, goal)
        ...
        soul.context.set(:__descartes_dynamic_jobs, queue)
      ...
      def setup_default_planner_if_missing!
        return if @agents.key?(:__descartes_planner)

        evaluator_profile = @engine_evaluator.instance_variable_get(:@profile_name) || "glm"
        agent :__descartes_planner,
              profile_name: evaluator_profile,
              max_turns: 50,
              system_prompt: "You are the primary Descartes Auto-Planner. Break down complex goals into execution steps using your `spawn_job` tool. Assign them to generic worker agents. Do not stop until all components of the goal are planned. Use update_context to return important facts.",
              tools: [:spawn_job]
      end
    end
  end
