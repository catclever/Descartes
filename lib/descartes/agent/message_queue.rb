  ...
  module Agent
    class MessageQueue
      ...
      def evict!
        ...
        @logger.warn "[Descartes::MessageQueue] Eviction Triggered: Token limit exceeded (#{current_tokens} > #{@eviction_threshold}). Compressing history."
        ...
        @logger.info "[Descartes::MessageQueue] Evicting #{evict_round_count}/#{total_rounds} rounds (#{evicted_messages.size} messages)"
        ...
        if @llm
          ...
          @logger.warn "[Descartes::MessageQueue] No LLM provided for summarization. Hard clipping history."
          @history = [seed_prompt] + kept_messages
        end

        @logger.info "[Descartes::MessageQueue] Eviction complete. New token size roughly: #{@token_monitor.count_message_tokens(@history)}"
      end
      ...
      def heartbeat_check!
        ...
        @logger.warn "[Descartes::MessageQueue] Heartbeat Warning: Injecting memory limit warning (#{current_tokens} tokens)."
        @history << {
          role: "user",
      ...
      def generate_summary(evicted_messages, existing_summary)
        transcript = evicted_messages.map(&:to_json).join("\n")
