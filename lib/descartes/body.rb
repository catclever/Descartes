  ...
  class Body
    ...
    def execute_loop!(soul)
      ...
      loop do
        ...
        if ready_jobs.empty? && running_jobs.empty? && @active_threads.empty?
          @logger.error "[Descartes::Body::#{@name}] DEADLOCK: No jobs are running and no pending jobs have dependencies met. Terminating."
          break
        end
    ...
    def intercept_dynamic_jobs!(soul)
      ...
      queue.each do |job_spec|
        ...
        @logger.info "[Descartes::Body] Intercepted Dynamic Job Spawn: :#{job_sym}"
